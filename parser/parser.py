...
class Parser:
    ...
    parse_html(keyword, html, registr) - возвращает количество совпадений слова в html
    """
    ...
    @classmethod
    def parse_html(cls, keyword, html, registr=False):
        """
        :param keyword: слово для поиска
        :param html: 
        :param registr: учитывать регистр или нет (по умолчанию - нет)
        ...
        return len(re.findall(r'\b' + keyword + r'\b', str(html)))
...
if __name__ == '__main__':
    ...
    count = 0
    for i, link in enumerate(site_links[:500]):
        try:
            p.open_page(link)
            count += Parser.parse_html(person_name, p.html)
            print('name {}%'.format(i/500*100))
        except Exception as e:
            print('try %d' % i)
            print('error url: %s' % link)
            print('Exeption: %s' % e)
